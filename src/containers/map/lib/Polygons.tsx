...
export class Component extends React.Component<PropsUnion, State> {
  ...
  public shouldComponentUpdate(
    ...
    return !(
      (this.props.zoom <= range[0] && nextProps.zoom <= range[0]) ||
  ...
  public render() {
    ...
    return (
      <>
        {overlap &&
          overlap.shapes.map((shape: any, i: any) =>
            this.renderPolygon(`overlap:${i}`, "#000", shape, overlapVisible)
          )}
        {travelTimes &&
          travelTimes.map((travelTime, i) =>
            travelTime.res.shapes.map((shape: any, j: any) =>
              this.renderPolygon(
  ...
  private renderPolygon(
    key: string,
    color: string,
    shape: NonNullable<StateProps["overlap"]>["shapes"][0],
    visible: boolean
  ) {
    const progress = Math.max(
      Math.min(1 - (this.props.zoom - range[0]) / (range[1] - range[0]), 1),
      0
    );

    if (
      shape.shell.length <=
      (minimumPolyRange[1] - minimumPolyRange[0]) * (1 - progress) +
        minimumPolyRange[0]
    )
      return null;
    ...
    return (
      <Polygon
        ...
        paths={[
          ...
          ...shape.holes
            .filter((hole: { length: number }) => hole.length > 15)
            .map((hole: { lat: number; lng: number }[]) =>
              this.getSmoothShape(hole, progress)
            ),
        ]}
        visible={visible}
      />
  ...
  private getSmoothShape(
    ...
    const simplifiedShape = simplify(
      ...
      0.01 * progress * Math.min(coordinates.length / 500, 1),
      false
    );
