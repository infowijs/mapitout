	...
	public render() {
		...
		return (
			...
			>
				<StyledTooltip>
					<StyledTooltipHeader>
						<StyledTooltipHeaderContent>{schoolDetailPin && schoolDetailPin.schools.length > 1 ? 'Schools' : 'School'}</StyledTooltipHeaderContent>
						...
						{schoolDetailPin && schoolDetailPin.schools.map((school) => (
							<StyledContentItem href={getValidUrl(school.url)} target='_blank'>
								<StyledContentItemIcon>
									...
									{school.type === 'mixed' && <img src={iconMixed} alt='Mixed school icon'/>}
								</StyledContentItemIcon>
...
export const SchoolDetailPin = connect<StateProps, DispatchProps, Props, ReduxState>(
	mapStateToProps,
	mapDispatchToProps
)(Component)
