...
type SimplifiedAddress = Pick<Address, 'lat'> & Pick<Address, 'lng'> & {
	id: string
	visible: boolean
	style: string
}
...
export class Component extends React.Component<PropsUnion, State> {
	...
	public shouldComponentUpdate(nextProps: Readonly<PropsUnion>, nextState: Readonly<State>, nextContext: any): boolean {
		...
		if (
			...
			|| this.props.onlyInternationalVisibility !== nextProps.onlyInternationalVisibility
		) {
			this.setState({
	...
	private trim = (addresses: Array<State['addresses'][0] & {visible: boolean}>): SimplifiedAddress[] => addresses
		.map((address) => {
			const numberOfSchools = address.schools.length
			const numberOfPrimaryOnlySchools = address.schools.filter((school) => school.type === 'primary').length
			const numberOfSecondaryOnlySchools = address.schools.filter((school) => school.type === 'secondary').length
			let style: string
